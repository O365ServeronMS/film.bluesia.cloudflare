...
- Image cache namespace `cf-img-jun-2026b` rejects oversized untransformed origin fallbacks. Do not cache multi-megabyte JPEGs under fixed profile keys; the edge cache key must include the image namespace so old edge `EDGE_HIT` objects are bypassed. Use `X-Film-Bluesia-Net-Image-Transform` to distinguish `transformed`, `origin-fallback`, and `rejected-large-origin`.
